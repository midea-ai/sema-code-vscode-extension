        ...
        <div className="section-card">
            <div className="section-card-header" onClick={() => setExpanded(!expanded)} style={{ cursor: 'pointer' }}>
                ...
                <span className="section-card-name">
                    <span style={{ color: 'var(--vscode-descriptionForeground)', fontWeight: 'normal', fontSize: '11px' }}>{task.id}</span> {task.cronToHuman}
                </span>
            ...
            {expanded && (
                <>
                    <div className="task-detail-info">
                        <div><strong>Cron:</strong> <span className="cron-expression-tag">{task.cron.split('').map((ch, i) => ch === '*' ? ch : <span key={i} className="cron-highlight">{ch}</span>)}</span> <span className="readonly-tab">{task.recurring ? '周期' : '一次性'}</span>{task.durable && <> <span className="readonly-tab">持久化</span></>}</div>
                    ...
                    <div className="cron-card-body">
                        ...
                        <pre className="task-detail-output">
                            {(() => {
                                ...
                                if (lines.length <= MAX_LINES) return task.prompt;
                                ...
                                return (<>
                                    ...
                                    <div
                                        ...
                                        onClick={() => vscode.postMessage({
                                            ...
                                            content: task.prompt,
                                            title: `${task.id} (${task.cronToHuman})`,
                                            toolId: task.id,
                                        })}
                                    >...省略了 {omitted} 行</div>
                                </>);
...
const CronTaskConfig: React.FC<CronTaskConfigProps> = ({ vscode, refreshTrigger, onCountChange }) => {
    ...
    const handleRefresh = () => {
        setRefreshing(true);
        vscode.postMessage({ command: 'refreshCronTasks' });
    };
