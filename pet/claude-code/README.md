...
让 Claude Code 用上桌宠 SemaPet：开会话时桌宠自动出现并跟随状态（思考 / 工作 / 等你确认 / 空闲），结束自动退出。靠 Claude Code 的 **hooks** 驱动，装一次即可。
...
`bootstrap.js` 是 `curl | node` 的一行入口，从 GitHub raw 把脚本拉到稳定路径 `~/.sema/pet/claude-code/` 再调 `install.js`；`install.js` 负责注册 hook（本地有 checkout 时也能直接 `node pet/claude-code/install.js` 跑）；`hook.js` 是分派器，按 Claude Code 事件（SessionStart/UserPromptSubmit/PreToolUse/Stop/SessionEnd 等）调 `lib/` 里的客户端，通过 `127.0.0.1:24700` 把状态发给桌宠进程。桌宠本体（三平台原生进程、zip 来源、协议）见 [`../README.md`](../README.md)。
